...
class EpochBatchIterator(object):
    ...
    def __init__(
        ...
        num_shards=1, shard_id=0, ignoredIndices=None, savedir=None,
    ):
        assert isinstance(dataset, FairseqDataset)
        self.dataset = dataset

        #up to here elements in source and target are collated in the sentence dimension
        ...
        # these are to handle difference in examples for seq and hier data formats
        self.savedir = savedir
        self.ignoredIndices = ignoredIndices if ignoredIndices is not None else []
    ...
    def _batch_generator(self):
        ...
        if len(ignored) > 0:
            ...
            if self.savedir:
                f=open(self.savedir + "ignoredIndices.log", 'w')
