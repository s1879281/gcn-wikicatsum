...
class SequenceGenerator(object):
    ...
    @classmethod
    def tostring(self, tensor, ignore=[]):
        """Helper for converting a tensor of token indices to a string.

        Can optionally remove BPE symbols or escape <unk> words.
        """
        if torch.is_tensor(tensor) and tensor.dim() == 2:
            return [SequenceGenerator.tostring(t, ignore=ignore) for t in tensor]


        sent = ' '.join(str(i.item()) for i in tensor if i not in ignore)  # if i != self.eos())
        return sent
    ...
    def __init__(
        ...
        sampling=False, sampling_topk=-1, sampling_temperature=1, flatdec=False,
    ):
    ...
    def generate(self, input, beam_size=None, maxlen=None, prefix_tokens=None):
        """Generate a batch of translations."""
        with torch.no_grad():
            return self._generate(input, beam_size, maxlen, prefix_tokens)

    def _get_encoder_output(self, input, beam_size):
        encoder_outs = []
        incremental_states = {}
        if not self.flatdec:
            ...
            for model in self.models:
                ...
                encoder_out = model.run_encoder_forward(input, srclen, ssentlen, beam_size,)
                encoder_outs.append(encoder_out)

        return encoder_outs, incremental_states

    def _generate(self, input, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        if not self.flatdec:
            ...
            bsz, srclen, ssentlen = src_tokens.size()
        encoder_outs, incremental_states = self._get_encoder_output(input, beam_size)

        # initialize buffers
        if self.flatdec:
            source_size = src_tokens.size(1)*src_tokens.size(2)
        else:
            source_size = src_tokens.size(1)
        scores = src_tokens.data.new(bsz * beam_size, maxlen + 1).float().fill_(0)
        scores_buf = scores.clone()
        tokens = src_tokens.data.new(bsz * beam_size, maxlen + 2).fill_(self.pad)
        tokens_buf = tokens.clone()
        tokens[:, 0] = self.eos
        attn = scores.new(bsz * beam_size, source_size, maxlen + 2)
        attn_buf = attn.clone()
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                def get_hypo():
                    ...
                    hypo_attn = attn_clone[i][nonpad_idxs.view(-1)] ##if we want to do atte on linearised hierarchical input, need to see this
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                for i, model in enumerate(self.models):
                    ...
                    if self.flatdec:
                        encoder_outs[i] = model.docEncoder.reorder_encoder_out(encoder_outs[i], reorder_state)
                    else:
                        encoder_outs[i] = model.encoder.reorder_encoder_out(encoder_outs[i], reorder_state)

            probs, avg_attn_scores, prevcontextv = self._decode(
                tokens[:, :step + 1], encoder_outs, incremental_states, prevcontextv)
            ...
            if len(finalized_sents) > 0:
                ...
                # my gosh
                if prevcontextv is not None:
                    reduced_prevhids = []
                    for t in prevcontextv:
                        t = t.view(bsz, -1)[batch_idxs].view(new_bsz * beam_size, -1).unsqueeze(1)
                        reduced_prevhids.append(t)
                    prevcontextv = reduced_prevhids
            ...
            if prevcontextv is not None:
                active_prevhids = []
                for t in prevcontextv:
                    t = torch.index_select(t, dim=0, index=active_bbsz_idx)
                    active_prevhids.append(t)
                prevcontextv = active_prevhids
    ...
    def _decode(self, tokens, encoder_outs, incremental_states, prev_contexts=None):
        if len(self.models) == 1:
            return self._decode_one(tokens, self.models[0], encoder_outs[0], incremental_states, log_probs=True, prev_contexts=prev_contexts)

        avg_probs = None
        avg_attn = None
        for model, encoder_out in zip(self.models, encoder_outs):
            probs, attn, new_prev_contexts = self._decode_one(tokens, model, encoder_out, incremental_states, log_probs=False, prev_contexts=prev_contexts)
        ...
        return avg_probs, avg_attn, new_prev_contexts

    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs, prev_contexts=None):
        with torch.no_grad():
        ...
        # add the following for dpp
        new_prev_contexts = None
        if len(decoder_out)==3:
            new_prev_contexts = decoder_out[2]
        return probs, attn, new_prev_contexts
