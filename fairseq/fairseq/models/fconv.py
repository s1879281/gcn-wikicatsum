...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, target_embedding, encoder_out, encoder_padding_mask):
        ...
        if encoder_padding_mask is None:
            ...
            s = s - encoder_padding_mask.type_as(x).sum(dim=1, keepdim=True)  # exclude padding; [b x s*w] --> [b x 1]
            allpadidxs = s.eq(0)
            s[allpadidxs] = 1 # to make normalization multiply by 1 (do nothing) in case of all padds (otherwise getting nan)
            s = s.unsqueeze(-1)
            x = x * (s * s.rsqrt())
...
class NonScalingAttentionLayer(AttentionLayer):
    """ L.P. done to try with hierarchical input encoders"""
    def forward(self, x, target_embedding, encoder_out, encoder_padding_mask):
        residual = x
...
class ExtendedAttentionLayer(nn.Module):
    def __init__(self, conv_channels, embed_dim, normalization_constant=0.5, bmm=None):
        super().__init__()
        self.normalization_constant = normalization_constant
        # projects from output of convolution to embedding dimension
    ...
    def forward(self, x, target_embedding, h0, encoder_out, encoder_padding_mask):
        residual = x

        # attention
        x = (self.in_projection(x) + target_embedding + h0) * math.sqrt(self.normalization_constant)
        x = self.bmm(x, encoder_out[0])
...
class WeigthedAttentionLayer(nn.Module):
    """L.P. done for hierarchical attention"""
    def __init__(self, conv_channels, embed_dim, normalization_constant=0.5, bmm=None):
        super().__init__()
        self.normalization_constant = normalization_constant
        # projects from output of convolution to embedding dimension
    ...
    def forward(self, x, target_embedding, encoder_out, encoder_padding_mask):
        ...
        return x, attn_scores #just return the weighted sum


class WeigthedAttentionLayer2(nn.Module):
    def __init__(self, conv_channels, embed_dim, normalization_constant=0.5, bmm=None):
        super().__init__()
        self.normalization_constant = normalization_constant
        # projects from output of convolution to embedding dimension
...
@register_model_architecture('fconv', 'fconv_wikicatsum_big')
...
@register_model_architecture('fconv', 'fconv_wikicatsum')
def fconv_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 256)
    args.encoder_layers = getattr(args, 'encoder_layers', '[(256, 3)] * 4')
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 256)
