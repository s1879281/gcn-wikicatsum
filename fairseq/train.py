...
def main(args):
    ...
    if args.outindicesValid:
        ignoredIndicesValid = []
        f=open(args.outindicesValid, 'r')
        for line in f.readlines():
            ignoredIndicesValid.append(int(line.strip()))
    ...
    while lr > args.min_lr and epoch_itr.epoch < max_epoch and trainer.get_num_updates() < max_update:
        # train for one epoch
        train(args, trainer, task, epoch_itr, ignoredIndicesValid=ignoredIndicesValid)

        if epoch_itr.epoch % args.validate_interval == 0:
            valid_losses = validate(args, trainer, task, epoch_itr, valid_subsets, ignoredIndicesValid=ignoredIndicesValid)
...
def train(args, trainer, task, epoch_itr, ignoredIndicesValid=None):
    ...
    for i, sample in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        if args.save_interval_updates > 0 and num_updates % args.save_interval_updates == 0:
            valid_losses = validate(args, trainer, task, epoch_itr, [first_valid], ignoredIndicesValid=ignoredIndicesValid)
            save_checkpoint(args, trainer, epoch_itr, valid_losses[0])
...
def validate(args, trainer, task, epoch_itr, subsets, ignoredIndicesValid=None):
